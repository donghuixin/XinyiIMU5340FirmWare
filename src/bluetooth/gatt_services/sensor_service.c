...
static K_THREAD_STACK_DEFINE(thread_stack_notify,
                             CONFIG_SENSOR_GATT_NOTIFY_STACK_SIZE);

K_MSGQ_DEFINE(gatt_queue, sizeof(struct sensor_data),
              CONFIG_SENSOR_GATT_SUB_QUEUE_SIZE, 4);
...
static void
sensor_config_status_ccc_cfg_changed(const struct bt_gatt_attr *attr,
                                     uint16_t value) {
...
static ssize_t write_config(struct bt_conn *conn,
  ...
  if (len != sizeof(struct sensor_config)) {
    LOG_WRN(
        "Write sensor config: Incorrect data length: Expected %i but got %i",
        sizeof(struct sensor_config), len);
  ...
  if (config->storageOptions == 0) {
    ...
    LOG_INF("Setup sensor ID %i with samplerateIndex %i", config->sensorId,
...
static ssize_t write_sensor_rec_name(struct bt_conn *conn,
  ...
  LOG_DBG("Attribute write, len: %u, handle: %u, conn: %p", len, attr->handle,
          (void *)conn);
  if (len > MAX_SENSOR_REC_NAME_LENGTH - 1) {
    LOG_WRN("Write sensor recording name: Data length exceeds maximum allowed "
            "length of %i",
            MAX_SENSOR_REC_NAME_LENGTH - 1);
    return BT_GATT_ERR(BT_ATT_ERR_INVALID_ATTRIBUTE_LEN);
  }
...
static ssize_t read_sensor_config_status(struct bt_conn *conn,
                                         ...
                                         uint16_t offset) {
  const uint16_t size =
      sizeof(struct sensor_config) * active_sensor_configs_size;
  ...
  return bt_gatt_attr_read(conn, attr, buf, len, offset, active_sensor_configs,
                           size);
}

BT_GATT_SERVICE_DEFINE(
    sensor_service, BT_GATT_PRIMARY_SERVICE(BT_UUID_SENSOR),
    BT_GATT_CHARACTERISTIC(BT_UUID_SENSOR_CONFIG, BT_GATT_CHRC_WRITE,
                           BT_GATT_PERM_WRITE, NULL, write_config, &config),
    BT_GATT_CHARACTERISTIC(BT_UUID_SENSOR_DATA, BT_GATT_CHRC_NOTIFY,
                           BT_GATT_PERM_NONE, NULL, NULL, &sensor_data),
...
int init_sensor_config_status() {
  ...
  active_sensor_configs =
      k_malloc(sizeof(struct sensor_config) * active_sensor_configs_size);
  ...
  for (size_t i = 0; i < active_sensor_configs_size; i++) {
    struct SensorScheme *sensor_scheme =
        getSensorSchemeForId(parse_info_scheme->sensorIds[i]);
    LOG_DBG("Initializing sensor config state for sensor with id %d",
            sensor_scheme->id);

    active_sensor_configs[i].sensorId = sensor_scheme->id;
    if (sensor_scheme->configOptions.availableOptions & FREQUENCIES_DEFINED) {
      active_sensor_configs[i].sampleRateIndex =
          sensor_scheme->configOptions.frequencyOptions.defaultFrequencyIndex;
    } else {
      active_sensor_configs[i].sampleRateIndex =
          0; // Default to 0 if frequencies are not defined
    }
...
int set_sensor_config_status(struct sensor_config config) {
  ...
  if (sensor_config_index >= 0) {
    ...
    struct sensor_config *new_active_sensor_configs =
        k_realloc(active_sensor_configs, active_sensor_configs_size);
  ...
  if (sensor_config_status_ntfy_enabled) {
    LOG_DBG("Sensor config status notification, notifying %zu active sensor "
            "configs",
            active_sensor_configs_size);
...
int init_sensor_service() {
  ...
  thread_id_notify = k_thread_create(
      ...
      CONFIG_SENSOR_GATT_NOTIFY_STACK_SIZE, (k_thread_entry_t)notification_task,
      NULL, NULL, NULL, K_PRIO_PREEMPT(CONFIG_SENSOR_GATT_NOTIFY_THREAD_PRIO),
      0, K_NO_WAIT);
  ...
  ret = zbus_chan_add_obs(&sensor_chan, &sensor_queue_listener,
  ...
  ret = zbus_chan_add_obs(&bt_mgmt_chan, &bt_mgmt_evt_listen2,
                          ZBUS_ADD_OBS_TIMEOUT_MS);
  if (ret) {
...
const char *get_sensor_recording_name() { return sensor_recording_name; }
