...
static struct bt_le_adv_param ext_adv_param = {
    ...
    .options = BT_LE_ADV_OPT_CONNECTABLE | BT_LE_ADV_OPT_USE_NAME | BT_LE_ADV_OPT_SCANNABLE,
...
static void filter_accept_list_add(const struct bt_bond_info *info,
                                   void *user_data) {
  int ret;
 ...
 *
 * @note The address of the remote device is only printed if directed
 * advertisement is active.
 */
static int addr_print(bt_addr_le_t const *const local_addr,
                      bt_addr_le_t const *const dir_adv_addr) {
  ...
  if (dir_adv_addr != NULL) {
    (void)bt_addr_le_to_str(dir_adv_addr, directed_to_addr_str,
                            BT_ADDR_LE_STR_LEN);
...
static int direct_adv_create(uint8_t ext_adv_index, bt_addr_le_t addr) {
  ...
  if (ret) {
    LOG_ERR("Failed to clear advertising data for set %d. Err: %d",
            ext_adv_index, ret);
    return ret;
  }
...
static int extended_adv_create(uint8_t ext_adv_index) {
  ...
  if (per_adv_local[ext_adv_index] != NULL && IS_ENABLED(CONFIG_BT_PER_ADV)) {
    ...
    ret =
        bt_le_per_adv_set_param(ext_adv[ext_adv_index], LE_AUDIO_PERIODIC_ADV);
    if (ret) {
      LOG_ERR("Failed to set periodic advertising parameters: %d", ret);
      return ret;
    }

    ret = bt_le_per_adv_set_data(ext_adv[ext_adv_index],
                                 per_adv_local[ext_adv_index],
...
static void advertising_process(struct k_work *work) {
  ...
  if (!k_msgq_get(&bonds_queue, &addr, K_NO_WAIT) && !dir_adv_timed_out) {
    ...
    ret = bt_le_ext_adv_start(ext_adv[ext_adv_index],
...
void bt_mgmt_dir_adv_timed_out(uint8_t ext_adv_index) {
  ...
  /* Use legacy advertising for Chrome Web Bluetooth compatibility */
  if (IS_ENABLED(CONFIG_BT_FILTER_ACCEPT_LIST)) {
    ret = bt_le_ext_adv_create(LE_AUDIO_LEGACY_ADV_CONN_NAME, &adv_cb,
                               &ext_adv[ext_adv_index]);
  } else {
    ret = bt_le_ext_adv_create(LE_AUDIO_LEGACY_ADV_CONN_NAME, &adv_cb,
                               &ext_adv[ext_adv_index]);
...
int bt_mgmt_adv_start(uint8_t ext_adv_index, const struct bt_data *adv,
                      size_t adv_size, const struct bt_data *per_adv,
  ...
  if (connectable) {
    ...
    ret = bt_le_ext_adv_create(LE_AUDIO_LEGACY_ADV_CONN_NAME, &adv_cb,
                               &ext_adv[ext_adv_index]);
    if (ret) {
      LOG_ERR("Unable to create a connectable advertising set: %d", ret);
      return ret;
    }
  } else {
    ret =
        bt_le_ext_adv_create(&ext_adv_param, &adv_cb, &ext_adv[ext_adv_index]);
    if (ret) {
...
void bt_mgmt_adv_init(void) { k_work_init(&adv_work, advertising_process); }
