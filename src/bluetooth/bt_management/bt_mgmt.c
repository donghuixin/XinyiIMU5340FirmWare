...
static void conn_params_updated(struct bt_conn *conn, uint16_t interval,
  ...
  LOG_INF("Conn params updated: interval %d unit, latency %d, timeout: %d0 ms",
...
static void connected_cb(struct bt_conn *conn, uint8_t err) {
  ...
  if (err == BT_HCI_ERR_ADV_TIMEOUT && IS_ENABLED(CONFIG_BT_PERIPHERAL)) {
    LOG_INF(
        "Directed adv timed out with no connection, reverting to normal adv");
...
static void disconnected_cb(struct bt_conn *conn, uint8_t reason) {
  ...
  /* The mutex for preventing the racing condition if two headset disconnected
   * too close, cause the disconnected_cb() triggered in short time leads to
   * duplicate scanning operation.
   */
...
static int local_identity_addr_print(void) {
  ...
  if (num_ids != CONFIG_BT_ID_MAX) {
    LOG_ERR("The default config supports %d ids, but %d was found",
            (int)CONFIG_BT_ID_MAX, (int)num_ids);
    return -ENOMEM;
  }
...
void bt_mgmt_num_conn_get(uint8_t *num_conn) {
  bt_conn_foreach(BT_CONN_TYPE_LE, conn_state_connected_check,
                  (void *)num_conn);
}
