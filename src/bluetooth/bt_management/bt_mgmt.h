...
#define LE_AUDIO_EXTENDED_ADV_CONN_NAME                                        \
  BT_LE_ADV_PARAM(                                                             \
      BT_LE_ADV_OPT_EXT_ADV | BT_LE_ADV_OPT_CONN | BT_LE_ADV_OPT_USE_NAME,     \
...
#define LE_AUDIO_PERIODIC_ADV                                                  \
  BT_LE_PER_ADV_PARAM(CONFIG_BLE_ACL_PER_ADV_INT_MIN,                          \
                      CONFIG_BLE_ACL_PER_ADV_INT_MAX, BT_LE_PER_ADV_OPT_NONE)

#define BT_LE_ADV_FAST_CONN                                                    \
  BT_LE_ADV_PARAM(BT_LE_ADV_OPT_CONN, BT_GAP_ADV_FAST_INT_MIN_1,               \
                  BT_GAP_ADV_FAST_INT_MAX_1, NULL)

/* Legacy connectable advertising with device name.
 * Chrome Web Bluetooth does NOT support Extended Advertising,
 * so we need this legacy variant for browser compatibility. */
#define LE_AUDIO_LEGACY_ADV_CONN_NAME                                          \
...
/**
 * @brief	Get the numbers of connected members of a given 'Set Identity
 * Resolving Key' (SIRK). The SIRK shall be set through bt_mgmt_scan_sirk_set()
 ...
 * @param[in]      data            Data to store in the buffer, can be a pointer
 * or value.
 *
 * @return	0 if success, error otherwise.
 */
int bt_mgmt_adv_buffer_put(struct bt_data *const adv_buf, uint32_t *index,
                           size_t adv_buf_vacant, size_t data_len, uint8_t type,
                           void *data);
 ...
 * @param[in]	name		Name to search for. Depending on @p type of
 * search, device name or broadcast name. Can be max BLE_SEARCH_NAME_MAX_LEN
 * long; everything beyond that value will be cropped. Can be NULL. Shall be
 * '\0' terminated.
 * @param[in]	brdcast_id	Broadcast ID to search for. Only valid if @p
 * type is BT_MGMT_SCAN_TYPE_BROADCAST. If both @p name and @p brdcast_id are
 *				provided, then brdcast_id will be used.
 *				Set to BRDCAST_ID_NOT_USED if not in use.
 *
 * @note	To restart scanning, call this function with all 0s and NULL,
 * except for @p type. The same scanning parameters as when bt_mgmt_scan_start
 * was last called will then be used.
 *
 * @return	0 if success, error otherwise.
 */
int bt_mgmt_scan_start(uint16_t scan_intvl, uint16_t scan_win,
                       enum bt_mgmt_scan_type type, char const *const name,
                       uint32_t brdcast_id);
 ...
 * @param[in]	ext_adv		The data to be put in the extended
 * advertisement.
 * @param[in]	ext_adv_size	Size of @p ext_adv.
 * @param[in]	per_adv		The data for the periodic advertisement; can be
 * NULL.
 * @param[in]	per_adv_size	Size of @p per_adv.
 * @param[in]	connectable	Specify if advertisement should be connectable
 * or not.
 *
 * @note	To restart advertising, call this function with all 0s and NULL,
 * except for connectable. The same advertising parameters as when
 * bt_mgmt_adv_start was last called will then be used.
 *
 * @return	0 if success, error otherwise.
 */
int bt_mgmt_adv_start(uint8_t ext_adv_index, const struct bt_data *ext_adv,
...
/**
 * @brief	Disconnect from a remote device or cancel the pending
 * connection.
 *
