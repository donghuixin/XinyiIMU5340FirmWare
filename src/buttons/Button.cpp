...
void Button::button_isr(const struct device *dev, struct gpio_callback *cb,
  ...
  button->_temp_buttonState =
      static_cast<button_action>(gpio_pin_get_dt(&(button->button)));
...
void Button::begin() {
  ...
  gpio_init_callback(&button_cb_data, button_isr,
                     button_cb_data.pin_mask | BIT(button.pin));
...
button_action Button::getState() const { return _buttonState; }
