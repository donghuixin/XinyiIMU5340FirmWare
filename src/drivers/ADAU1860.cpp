...
int ADAU1860::begin() {
  ...
  writeReg(registers::PMU_CTRL2, &startup_dlycnt_byp,
           sizeof(startup_dlycnt_byp));
  ...
  // Power saving
  uint8_t power_mode =
      0x40 | 0x01 | 0x04; // CM_Startup_over | Hibernate 1, Master enable
  writeReg(registers::CHIP_PWR, &power_mode, sizeof(power_mode));
...
int ADAU1860::setup_DAC() {
  ...
  writeReg(registers::ADC_DAC_HP_PWR, &headphone_power,
           sizeof(headphone_power));
...
int ADAU1860::setup_FDSP() {
  ...
  for (int i = 0; i < FDSP_NUM_PARAMS; i++) {
    writeReg(FDSP_BANK_A(i), (uint8_t *)fdsp_param_bank_a[i],
             sizeof(fdsp_param_bank_a[i]));
    writeReg(FDSP_BANK_B(i), (uint8_t *)fdsp_param_bank_b[i],
             sizeof(fdsp_param_bank_b[i]));
    writeReg(FDSP_BANK_C(i), (uint8_t *)fdsp_param_bank_c[i],
             sizeof(fdsp_param_bank_c[i]));
  }
...
int ADAU1860::fdsp_safe_load(sl_address address, safe_load_params params,
  ...
  if (update_inactive) {
    ...
    for (int i = 0; i < FDSP_NUM_BANKS; i++) {
      if (i == _active_bank)
        continue;
      writeReg(FDSP_BANK(i, address), (uint8_t *)params,
               sizeof(safe_load_params));
    }
  }

  uint8_t _address = address;
  writeReg(registers::FDSP_SL_ADDR, &_address, sizeof(_address));
  writeReg(registers::FDSP_SL_P0_0, (uint8_t *)params,
           sizeof(safe_load_params));
...
int ADAU1860::fdsp_set_volume(uint8_t volume) {
  float _val = ((float)(1 << 27)) *
               powf(10.f, -3.f * (((float)(0xFF - volume)) / 255.f));
...
int cmd_dsp_noise_gate(const struct shell *shell, size_t argc, char **argv) {
  if (argc != 6) {
    shell_print(shell, "Usage: dsp_noise_gate <threshold> <gain_min> "
                       "<gain_max> <attack> <release>");
    return -EINVAL;
  }
...
SHELL_STATIC_SUBCMD_SET_CREATE(dsp_cmd,
                               SHELL_COND_CMD(CONFIG_SHELL, noise_gate, NULL,
                                              "Set DSP noise gate parameters",
                                              cmd_dsp_noise_gate),
