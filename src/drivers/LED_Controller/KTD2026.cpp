...
void KTD2026::writeReg(uint8_t reg, uint8_t *buffer, uint16_t len) {
        ...
        if (ret) LOG_ERR("KTD2026 I2C write reg=0x%02x failed: %d", reg, ret);
...
void KTD2026::begin() {
	...
	_active = true;
        ...
        if (ret != 0) {
                LOG_ERR("KTD2026: pm_device_runtime_get(ls_1_8) failed: %d", ret);
        }

        ret = pm_device_runtime_get(ls_3_3);
        if (ret != 0) {
                LOG_ERR("KTD2026: pm_device_runtime_get(ls_3_3) failed: %d", ret);
        }
