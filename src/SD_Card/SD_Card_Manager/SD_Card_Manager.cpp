...
K_WORK_DELAYABLE_DEFINE(SDCardManager::unmount_work,
                        SDCardManager::unmount_work_handler);

void SDCardManager::sd_card_state_change_isr(const struct device *dev,
...
void SDCardManager::init() {
  ...
  gpio_init_callback(&sd_state_cb, sd_card_state_change_isr,
                     sd_state_cb.pin_mask | BIT(sd_state_pin.pin));
...
int SDCardManager::open_file(std::string path, bool write, bool append,
                             bool create) {
  if (!this->mounted) {
    LOG_ERR("SD card not mounted! Call SDCardManager::mount() first!");
    return -ENODEV;
...
ssize_t SDCardManager::write(std::string path, char *buf, size_t *buf_size,
                             bool append) {
...
int SDCardManager::open_write_close(std::string path, char *buf,
                                    size_t *buf_size, bool append,
                                    bool create) {
...
int SDCardManager::open_read_close(std::string path, char *buf,
                                   size_t *buf_size) {
