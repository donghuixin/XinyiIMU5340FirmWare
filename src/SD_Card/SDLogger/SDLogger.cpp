...
K_MSGQ_DEFINE(sd_sensor_queue, sizeof(sensor_data),
              CONFIG_SENSOR_SD_SUB_QUEUE_SIZE, 4);
...
static struct k_poll_event logger_evt = K_POLL_EVENT_INITIALIZER(
    K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY, &logger_sig);
...
void sensor_listener_cb(const struct zbus_channel *chan) {
  ...
  if (msg->sd) {
    /*if (!_prio_boost) {
        if (k_msgq_num_free_get(&sd_sensor_queue) <
    CONFIG_SENSOR_SD_SUB_QUEUE_SIZE / 2) { k_thread_priority_set(thread_id,
    K_PRIO_PREEMPT(CONFIG_SENSOR_SD_THREAD_PRIO - 1));
        ...
        if (k_msgq_num_used_get(&sd_sensor_queue) == 0) { // <
    CONFIG_SENSOR_SD_SUB_QUEUE_SIZE / 4 k_thread_priority_set(thread_id,
    K_PRIO_PREEMPT(CONFIG_SENSOR_SD_THREAD_PRIO)); _prio_boost = false;
...
void sd_listener_callback(const struct zbus_channel *chan) {
  const struct sd_msg *sd_msg_event =
      (sd_msg *)zbus_chan_const_msg(&sd_card_chan);
...
int SDLogger::init() {
  ...
  thread_id = k_thread_create(
      &thread_data, thread_stack, CONFIG_SENSOR_SD_STACK_SIZE,
      (k_thread_entry_t)sensor_sd_task, NULL, NULL, NULL,
      K_PRIO_PREEMPT(CONFIG_SENSOR_SD_THREAD_PRIO), 0, K_NO_WAIT);
  ...
  ret = zbus_chan_add_obs(&sensor_chan, &sensor_data_listener,
  ...
  ret = zbus_chan_add_obs(&sd_card_chan, &sd_card_event_listener,
                          ZBUS_ADD_OBS_TIMEOUT_MS);
  if (ret) {
 ...
 * Returns -EBUSY if logger is already open or -ENODEV if SD card not
 * initialized.
 */
...
int SDLogger::write_sensor_data(const void *const *data_blocks,
  ...
  if (left_space < total_length) {
    k_mutex_unlock(&write_mutex);
    LOG_WRN(
        "Not enough space in ring buffer: %d bytes needed, %d bytes available",
        total_length, left_space);
    return -ENOSPC;
  }
  ...
  for (size_t i = 0; i < block_count; i++) {
    ...
    if (written < lengths[i]) {
      k_mutex_unlock(&write_mutex);
      LOG_ERR(
          "Failed to write data block: %d bytes written, %d bytes requested",
          written, lengths[i]);
      return -ENOSPC;
    }
  }
...
int SDLogger::write_sensor_data(const sensor_data &msg) {
  const size_t data_size =
      sizeof(msg.id) + sizeof(msg.size) + sizeof(msg.time) + msg.size;
...
bool SDLogger::is_active() { return is_open; }
