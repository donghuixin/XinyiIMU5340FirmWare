...
#if IS_ENABLED(CONFIG_NRF5340_AUDIO_CS47L63_DRIVER)
...
static const struct device *const ls_1_8 = NULL;
static const struct device *const ls_3_3 = NULL;
static const struct device *const ls_sd = NULL;

static int load_switch_runtime_enable(const struct device *dev,
                                      const char *name) {
  if (dev == NULL) {
    LOG_DBG("Load switch %s not defined, skip enable", name);
    return 0;
  }
...
static int load_switch_suspend(const struct device *dev, const char *name) {
  if (dev == NULL) {
    LOG_DBG("Load switch %s not defined, skip suspend", name);
    return 0;
  }
...
// K_TIMER_DEFINE(PowerManager::charge_timer,
// PowerManager::charge_timer_handler, NULL);

K_WORK_DELAYABLE_DEFINE(PowerManager::charge_ctrl_delayable,
                        PowerManager::charge_ctrl_work_handler);

K_WORK_DELAYABLE_DEFINE(PowerManager::power_down_work,
                        PowerManager::power_down_work_handler);

// K_WORK_DEFINE(PowerManager::power_down_work,
// PowerManager::power_down_work_handler);
// K_WORK_DEFINE(PowerManager::charge_ctrl_work,
// PowerManager::charge_ctrl_work_handler);
K_WORK_DEFINE(PowerManager::fuel_gauge_work,
...
// LoadSwitch
// PowerManager::v1_8_switch(GPIO_DT_SPEC_GET(DT_NODELABEL(load_switch),
// gpios));

void PowerManager::fuel_gauge_callback(const struct device *dev,
...
void PowerManager::battery_controller_callback(const struct device *dev,
                                               struct gpio_callback *cb,
                                               uint32_t pins) {
  k_work_submit(&battery_controller_work);
}

void PowerManager::power_good_callback(const struct device *dev,
                                       struct gpio_callback *cb,
                                       uint32_t pins) {
...
void PowerManager::fuel_gauge_work_handler(struct k_work *work) {
  ...
  case 1:
    ...
    if (current >
        0.8 * target_current - 2 * power_manager._battery_settings.i_term) {
  ...
  case 3:
    ...
    if (fault & (1 << 5)) {
      ...
      if (power_connected &&
          current > 0.5 * power_manager._battery_settings.i_term) {
        msg.charging_state = PRECHARGING;
      }
    ...
    if ((ts_fault >> 5) & 0x7) {
      LOG_WRN("TS_ENABLED: %i, TS FAULT: %i", ts_fault >> 7,
              (ts_fault >> 5) & 0x3);
  ...
  if (msg.charging_state == FAULT || msg.charging_state == POWER_CONNECTED) {
    power_manager.chrg_interval =
        K_SECONDS(CONFIG_BATTERY_CHARGE_CONTROLLER_FAST_INTERVAL_SECONDS);
  } else {
    power_manager.chrg_interval =
        K_SECONDS(CONFIG_BATTERY_CHARGE_CONTROLLER_NORMAL_INTERVAL_SECONDS);
  }
...
int PowerManager::begin() {
  ...
  /* Meow Sense Tag: always power on �?no earable button logic needed */
  power_on = true;
  ...
  if (charging) {
    ...
    int ret = load_switch_runtime_enable(ls_1_8, "1.8V");
    if (ret != 0) {
      LOG_WRN("Error setting up load switch 1.8V.");
    }

    ret = load_switch_runtime_enable(ls_3_3, "3.3V");
    if (ret != 0) {
  ...
  ret = load_switch_runtime_enable(ls_sd, "SD");
  if (ret != 0) {
...
bool PowerManager::check_battery() {
  ...
  if (charging) {
    ...
    if (temp < _battery_settings.temp_min ||
        temp > _battery_settings.temp_max) {
      // set params
      battery_controller.disable_charge();
      return false;
    } else if (temp < _battery_settings.temp_fast_min ||
               temp > _battery_settings.temp_fast_max) {
      // set params
...
void PowerManager::get_battery_status(battery_level_status &status) {
  ...
  if (battery_controller.power_connected()) {
    ...
    } else if (charging_state == 0x2)
      status.power_state |= (0x3 << 5); // inactive discharge
  } else {
  ...
  // charge level
  if (gs.edv1)
    status.power_state |= (0x3 << 7); // critical
  else if (gs.edv2)
    status.power_state |= (0x2 << 7); // low
  else
    status.power_state |= (0x1 << 7); // good
...
int PowerManager::power_down(bool fault) {
  ...
  ret = load_switch_suspend(ls_sd, "SD");
  ret = load_switch_suspend(ls_3_3, "3.3V");
  ret = load_switch_suspend(ls_1_8, "1.8V");
  ...
  /*const struct device *const watch_dog =
  DEVICE_DT_GET(DT_CHOSEN(zephyr_bt_hci_rpmsg_ipc)); ret =
  pm_device_action_run(watch_dog, PM_DEVICE_ACTION_SUSPEND); ERR_CHK(ret);*/
...
int cmd_setup_fuel_gauge(const struct shell *shell, size_t argc,
                         const char **argv) {
  ARG_UNUSED(argc);
  ARG_UNUSED(argv);
...
static int cmd_battery_info(const struct shell *shell, size_t argc,
                            const char **argv) {
  ARG_UNUSED(argc);
  ARG_UNUSED(argv);
  ...
  shell_print(shell, "  Temperature: %.1f °C",
              (double)fuel_gauge.temperature());
  shell_print(shell, "  Current: %.1f mA (avg: %.1f mA)",
              (double)fuel_gauge.current(),
              (double)fuel_gauge.average_current());
  shell_print(shell, "  State of Charge: %.1f%%",
              (double)fuel_gauge.state_of_charge());

  // Capacity info
  shell_print(shell, "Capacity Information:");
  shell_print(shell, "  Design Capacity: %.1f mAh",
              (double)fuel_gauge.design_cap());
  shell_print(shell, "  Full Charge Capacity: %.1f mAh",
              (double)fuel_gauge.capacity());
  shell_print(shell, "  Remaining Capacity: %.1f mAh",
              (double)fuel_gauge.remaining_cap());

  // Time estimates
  float ttf = fuel_gauge.time_to_full();
  float tte = fuel_gauge.time_to_empty();
  shell_print(shell, "Time Estimates:");
  shell_print(shell, "  Time to Full: %ih %02dmin", (int)ttf / 60,
              (int)ttf % 60);
  shell_print(shell, "  Time to Empty: %ih %02dmin", (int)tte / 60,
              (int)tte % 60);
...
SHELL_STATIC_SUBCMD_SET_CREATE(battery_cmd,
                               SHELL_COND_CMD(CONFIG_SHELL, info, NULL,
                                              "Print battery info",
                                              cmd_battery_info),
                               SHELL_COND_CMD(CONFIG_SHELL, setup, NULL,
                                              "Setup fuel gauge",
                                              cmd_setup_fuel_gauge),
                               SHELL_SUBCMD_SET_END);

SHELL_CMD_REGISTER(battery, &battery_cmd, "Power Manager Commands", NULL);

/* DIAG */ static struct _diag_pm_ctor {
} _diag_pm;
PowerManager power_manager;
/* DIAG */ static struct _diag_pm_done {
} _diag_pm2;
