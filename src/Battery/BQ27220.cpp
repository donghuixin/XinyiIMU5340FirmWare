...
bool BQ27220::readReg(uint8_t reg, uint8_t *buffer, uint16_t len) {
  int ret;
  uint64_t now = micros();
  int delay = MIN(BQ27220_I2C_TIMEOUT_US - (int)(now - last_i2c),
                  BQ27220_I2C_TIMEOUT_US);
...
void BQ27220::enter_config_update() {
  ...
  do {
    ...
    k_msleep(100);
    timeout--;
  } while (!state.CFG_UPDATE && timeout > 0);
  if (timeout <= 0) {
    LOG_ERR("Failed to enter CONFIG UPDATE MODE (Timeout)!");
  } else {
...
void BQ27220::exit_config_update(bool init) {
  ...
  if (init)
    write_command(CONFIG_UPDATE_EXIT);
  else
    write_command(CONFIG_UPDATE_EXIT_NO_INIT);
  int timeout = 20;
  do {
    state = operation_state();
    k_msleep(100);
  ...
  if (timeout <= 0) {
    LOG_ERR("Failed to exit CONFIG UPDATE MODE (Timeout)!");
  } else {
...
int BQ27220::write_RAM(uint16_t ram_address, uint8_t *data, int len,
                       bool check) {
  uint8_t check_sum = 0;
  uint8_t data_len = 0;
  uint8_t buf[len];
...
void BQ27220::setup(const battery_settings &_battery_settings, bool init) {
  ...
  */
  ...
  // sysDown set Voltage
  ret = write_RAM(0x9240, _battery_settings.u_vlo * 1000 +
                              CONFIG_BATTERY_SYSDOWN_SET_OFFSET);

  // sysDown clear Voltage
  ret = write_RAM(0x9243, _battery_settings.u_vlo * 1000 +
                              CONFIG_BATTERY_SYSDOWN_SET_OFFSET +
                              CONFIG_BATTERY_SYSDOWN_HYSTERESIS);

  // FD set
  ret = write_RAM(0x9282, _battery_settings.u_vlo * 1000 +
                              CONFIG_BATTERY_FD_SET_OFFSET);

  // FD clear
  ret = write_RAM(0x9284, _battery_settings.u_vlo * 1000 +
                              CONFIG_BATTERY_FD_SET_OFFSET +
                              CONFIG_BATTERY_FD_HYSTERESIS);

  // FC Voltage
  ret = write_RAM(0x9288, _battery_settings.u_term * 1000 -
                              CONFIG_BATTERY_FC_VOLTAGE_OFFSET);
...
int BQ27220::set_int_callback(gpio_callback_handler_t handler) {
  gpio_init_callback(&int_cb_data, handler,
                     int_cb_data.pin_mask | BIT(gpout_pin.pin));
