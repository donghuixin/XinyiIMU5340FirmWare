...
BQ25120a::BQ25120a(TWIM *i2c)
    : _i2c(i2c) { //,
                  //load_switch(LoadSwitch(GPIO_DT_SPEC_GET(DT_NODELABEL(bq25120a),
                  //lsctrl_gpios))) {
}
...
bool BQ25120a::readReg(uint8_t reg, uint8_t *buffer, uint16_t len) {
  int ret;
  uint64_t now = micros();
  int delay = MIN(BQ25120a_I2C_TIMEOUT_US - (int)(now - last_i2c),
                  BQ25120a_I2C_TIMEOUT_US);
  int delay_hz = MIN(BQ25120a_HIGH_Z_TIMEOUT_US - (int)(now - last_high_z),
                     BQ25120a_HIGH_Z_TIMEOUT_US);
...
void BQ25120a::writeReg(uint8_t reg, uint8_t *buffer, uint16_t len) {
  ...
  if (delay > 0)
    k_usleep(delay); // TODO: assert no message ?

  _i2c->aquire();

  ret = i2c_burst_write(_i2c->master, address, reg, buffer, len);
  if (ret)
    LOG_WRN("BQ25120A I2C write reg=0x%02x failed: %d", reg, ret);
...
void BQ25120a::setup(const battery_settings &_battery_settings) {
  ...
  write_LS_control(true); // Enable LDO/LS output (EN_LS=1 in reg 0x07)
...
chrg_state BQ25120a::read_charging_control() {
  uint8_t status = 0;
  bool ret =
      readReg(registers::CHARGE_CTRL, (uint8_t *)&status, sizeof(status));
...
float BQ25120a::read_ldo_voltage() {
  uint8_t status = 0;
  bool ret =
      readReg(registers::LS_LDO_CTRL, (uint8_t *)&status, sizeof(status));
...
float BQ25120a::read_battery_voltage_control() {
  uint8_t status = 0;
  bool ret =
      readReg(registers::BAT_VOL_CTRL, (uint8_t *)&status, sizeof(status));
...
int BQ25120a::set_power_connect_callback(gpio_callback_handler_t handler) {
  gpio_init_callback(&power_connect_cb_data, handler,
                     power_connect_cb_data.pin_mask | BIT(pg_pin.pin));
...
int BQ25120a::set_int_callback(gpio_callback_handler_t handler) {
  gpio_init_callback(&int_cb_data, handler,
                     int_cb_data.pin_mask | BIT(int_pin.pin));
