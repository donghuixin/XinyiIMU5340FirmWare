...
K_THREAD_STACK_DEFINE(button_msg_sub_thread_stack,
                      CONFIG_BUTTON_MSG_SUB_STACK_SIZE);

#ifdef CONFIG_NRF5340_AUDIO
static struct k_thread le_audio_msg_sub_thread_data;
static k_tid_t le_audio_msg_sub_thread_id;
K_THREAD_STACK_DEFINE(le_audio_msg_sub_thread_stack,
...
static int zbus_subscribers_create(void) {
  ...
  button_msg_sub_thread_id = k_thread_create(
      ...
      NULL, NULL, NULL, K_PRIO_PREEMPT(CONFIG_BUTTON_MSG_SUB_THREAD_PRIO), 0,
      K_NO_WAIT);
...
static int zbus_link_producers_observers(void) {
  ...
  ret =
      zbus_chan_add_obs(&button_chan, &button_evt_sub, ZBUS_ADD_OBS_TIMEOUT_MS);
  if (ret) {
...
#ifdef CONFIG_NRF5340_AUDIO
  ret =
      zbus_chan_add_obs(&volume_chan, &volume_evt_sub, ZBUS_ADD_OBS_TIMEOUT_MS);
  if (ret) {
...
/*
 * @brief  The following configures the data for the extended advertising. This
 * includes the Audio Stream Control Service requirements [BAP 3.7.2.1.1] in the
 * AUX_ADV_IND Extended Announcements.
 *
 * @param  ext_adv_buf       Pointer to the bt_data used for extended
 * advertising.
 * @param  ext_adv_buf_size  Size of @p ext_adv_buf.
 * @param  ext_adv_count     Pointer to the number of elements added to @p
 * adv_buf.
 *
 * @return  0 for success, error otherwise.
 */
...
#ifdef CONFIG_NRF5340_AUDIO
static int ext_adv_populate(struct bt_data *ext_adv_buf,
                            size_t ext_adv_buf_size, size_t *ext_adv_count) {
  int ret;
  ...
  ret = bt_mgmt_manufacturer_uuid_populate(&uuid_buf,
  ...
  LOG_DBG("Size of adv data: %d, num_elements: %d",
          sizeof(struct bt_data) * ext_adv_buf_cnt, ext_adv_buf_cnt);
...
uint8_t stream_state_get(void) { return strm_state; }
...
void streamctrl_send(void const *const data, size_t size, uint8_t num_ch) {
  ...
  struct le_audio_encoded_audio enc_audio = {
      .data = data, .size = size, .num_ch = num_ch};
...
#define BT_CSIP_PADDED_RAND_SIZE                                               \
  (BT_CSIP_CRYPTO_PADDING_SIZE + BT_CSIP_CRYPTO_PRAND_SIZE)
...
static void write_sirk(uint32_t sirk) {
  ...
  if (ret == 0)
    ...
    LOG_ERR("UICR writing error: %i", ret);
}
...
static void device_found(const bt_addr_le_t *addr, int8_t rssi, uint8_t type,
  ...
  while (ad->len > 0) {
    ...
    if (is_le_audio_device && ad_type == BT_DATA_MANUFACTURER_DATA) {
      memset(chip_id, 0, sizeof(chip_id));
      memcpy(chip_id, data, sizeof(chip_id));
    }

    if (ad_type == BT_DATA_CSIS_RSI) {
  ...
  if (is_le_audio_device) {
    ...
    if (channel == AUDIO_CH_L) {
      ...
      // r' = padding || r
      (void)memset(res + BT_CSIP_CRYPTO_PRAND_SIZE, 0,
                   BT_CSIP_CRYPTO_PADDING_SIZE);
...
int streamctrl_start() // streamctrl_start
{
  ...
  ret = bt_mgmt_init();
  if (ret) {
    LOG_ERR("Fail to init BLE %d", ret);
    return ret;
  }

  ret = zbus_subscribers_create();
  if (ret) {
    LOG_ERR("Fail to create zbus_subscribers_create");
    return ret;
  }

  ret = zbus_link_producers_observers();
  if (ret) {
    LOG_ERR("Fail to link zbus channels_observers");
    return ret;
  }
  ...
  if (ret) {
    LOG_ERR("Fail to create audio_datapath_init");
    return ret;
  }
  ...
  struct bt_le_scan_param *scan_param = BT_LE_SCAN_PARAM(
      NRF5340_AUDIO_GATEWAY_SCAN_TYPE, BT_LE_SCAN_OPT_FILTER_DUPLICATE, 32, 32);
