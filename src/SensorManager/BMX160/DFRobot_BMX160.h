 ...
 * @brief DFRobot_BMI160 class infrastructure
...
#define BMX160_ACCEL_MG_LSB_2G                                                 \
  0.000061035F ///< Macro for mg per LSB at +/- 2g sensitivity (1 LSB =
               ///< 0.000061035mg) */
#define BMX160_ACCEL_MG_LSB_4G                                                 \
  0.000122070F ///< Macro for mg per LSB at +/- 4g sensitivity (1 LSB =
               ///< 0.000122070mg) */
#define BMX160_ACCEL_MG_LSB_8G                                                 \
  0.000244141F ///< Macro for mg per LSB at +/- 8g sensitivity (1 LSB =
               ///< 0.000244141mg) */
#define BMX160_ACCEL_MG_LSB_16G                                                \
  0.000488281F ///< Macro for mg per LSB at +/- 16g sensitivity (1 LSB =
               ///< 0.000488281mg) */

#define BMX160_GYRO_SENSITIVITY_125DPS                                         \
  0.0038110F ///< Gyroscope sensitivity at 125dps */
#define BMX160_GYRO_SENSITIVITY_250DPS                                         \
  0.0076220F ///< Gyroscope sensitivity at 250dps */
#define BMX160_GYRO_SENSITIVITY_500DPS                                         \
  0.0152439F ///< Gyroscope sensitivity at 500dps */
#define BMX160_GYRO_SENSITIVITY_1000DPS                                        \
  0.0304878F ///< Gyroscope sensitivity at 1000dps */
#define BMX160_GYRO_SENSITIVITY_2000DPS                                        \
  0.0609756F ///< Gyroscope sensitivity at 2000dps */
...
#define BMX160_MAGN_UT_LSB                                                     \
  (0.3F) ///< Macro for micro tesla (uT) per LSB (1 LSB = 0.1uT) */
...
typedef struct {
  ...
  uint16_t
      length; /**< While calling the API  "BMX160_get_fifo_data" , length stores
  ...
  uint16_t gyroByteStartIdx;  /**< Will be equal to length when no more frames
                                 are there to parse */
  uint16_t magnByteStartIdx;  /**< Will be equal to length when no more frames
                                 are there to parse */
 ...
 */
typedef enum {
  eBmx160BothAnySigMotionDisabled =
...
/* type definitions */
typedef int8_t (*bmx160ComFptrT)(uint8_t dev_addr, uint8_t reg_addr,
...
typedef struct {
  ...
  sBmx160Cfg_t prevMagnCfg;  /**< Structure to hold previous/old magn config
                                parameters.This is used at driver level to
                                prevent overwriting of same data, hence user does
                                not change it in the code */
  sBmx160Cfg_t accelCfg;     /**< Structure to configure Accel sensor */
  sBmx160Cfg_t prevAccelCfg; /**< Structure to hold previous/old accel config
                                parameters.This is used at driver level to
  ...
  sBmx160Cfg_t
      prevGyroCfg; /**< Structure to hold previous/old gyro config parameters.
                      This is used at driver level to prevent overwriting of
                      same data, hence user does not change it in the code */
...
typedef struct {
  ...
  uint16_t outputMode
      : 1; /**< 0 - push-pull 1- open drain,only valid if outputEn is set 1 */
  uint16_t outputType
      : 1; /**< 0 - active low , 1 - active high level.if outputEn is 1,this
              applies to interrupts,else PMU_trigger */
...
typedef struct {
#if LITTLE_ENDIAN == 1
  uint16_t stepDetectorEn
      : 1; /**< 1- step detector enable, 0- step detector disable */
...
typedef struct {
  ...
  uint16_t noMotionSel
      : 1; /**< no motion sel , 1 - enable no-motion ,0- enable slow-motion */
...
#if LITTLE_ENDIAN == 1
  uint16_t orientMode
      : 2; /**< thresholds for switching between the different orientations */
...
typedef struct {
  ...
  uint16_t flatHoldTime
      : 2; /**< delay time for which the flat value must remain stable for the
 ...
 */
typedef enum {
  eAccelRange_2G = 0b0011,  /**< Macro for mg per LSB at +/- 2g sensitivity (1
                               LSB = 0.000061035mg) */
  eAccelRange_4G = 0b0101,  /**< Macro for mg per LSB at +/- 4g sensitivity (1
                               LSB = 0.000122070mg) */
  eAccelRange_8G = 0b1000,  /**< Macro for mg per LSB at +/- 8g sensitivity (1
                               LSB = 0.000244141mg) */
  eAccelRange_16G = 0b1100, /**< Macro for mg per LSB at +/- 16g sensitivity (1
                               LSB = 0.000488281mg) */
} eAccelRange_t;
...
public:
  ...
  /**
   * @fn begin_async
   * @brief Start non-blocking initialization of the sensor
   * It resets the sensor and issues the Normal Mode commands for Accel, Gyro,
   * and Mag.
   *
   * @return true Success sending commands
   * @return false Fail
   */
  bool begin_async();
  ...
  /**
   * @fn check_pmu_status
   * @brief Check if Gyroscope has entered normal mode
   * @param pmu_val Pointer to store the PMU status register value
   * @param err_val Pointer to store the Error register value
   * @return true if Gyro is in normal mode, false otherwise
   */
  bool check_pmu_status(uint8_t *pmu_val, uint8_t *err_val);

  /**
   * @fn retry_gyro_mag_wakeup
   * @brief Resend Gyro and Mag normal mode commands
   */
  void retry_gyro_mag_wakeup();
   ...
   * @param bits
   * @n       eAccelRange_2G        Macro for mg per LSB at +/- 2g sensitivity
   * (1 LSB = 0.000061035mg)
   * @n       eAccelRange_4G        Macro for mg per LSB at +/- 4g sensitivity
   * (1 LSB = 0.000122070mg)
   * @n       eAccelRange_8G        Macro for mg per LSB at +/- 8g sensitivity
   * (1 LSB = 0.000244141mg)
   * @n       eAccelRange_16G       Macro for mg per LSB at +/- 16g sensitivity
   * (1 LSB = 0.000488281mg)
   */
   ...
   */
  void getAllData(sBmx160SensorData_t *magn, sBmx160SensorData_t *gyro,
