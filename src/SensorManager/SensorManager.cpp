...
static struct k_poll_event sensor_manager_evt = K_POLL_EVENT_INITIALIZER(
    K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY, &sensor_manager_sig);
...
K_THREAD_STACK_DEFINE(sensor_publish_thread_stack,
                      CONFIG_SENSOR_PUB_STACK_SIZE);
...
void init_sensor_manager() {
  ...
  sensor_pub_id = k_thread_create(
      &sensor_publish, sensor_publish_thread_stack,
      CONFIG_SENSOR_PUB_STACK_SIZE, sensor_chan_update, NULL, NULL, NULL,
  ...
  // =========================================================================
  // [終極修復] 直接在這裡進行 IMU 的同步硬體初始化
  // 確保只要 SensorManager 活著，IMU 就一定會跑完喚醒流程
  // =========================================================================
  LOG_INF("=== [SensorManager] Starting Synchronous IMU Hardware Init ===");
  EdgeMlSensor *imu_sensor = get_sensor(ID_IMU);
  if (imu_sensor != NULL) {
    imu_sensor->init(&sensor_queue);
  }
  LOG_INF("=== [SensorManager] Synchronous IMU Hardware Init Complete ===");
}
...
void start_sensor_manager() {
  ...
  // k_work_queue_unplug(&sensor_work_q); // REMOVED: unplugging permanently
  // stops the queue from processing work!
...
static void config_work_handler(struct k_work *work) {
  ...
  float sampleRate =
      getSampleRateForSensorId(config.sensorId, config.sampleRateIndex);
  if (sampleRate <= 0) {
    LOG_ERR("Invalid sample rate %f for sensor %i", (double)sampleRate,
            config.sensorId);
    return;
  }
  ...
  if (config.storageOptions & DATA_STREAMING) {
    ble_sensors.insert(config.sensorId);
