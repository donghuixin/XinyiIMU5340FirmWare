...
const SampleRateSetting<16> PPG::sample_rates = {
    ...
    {25, 50, 84, 100, 200, 400, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096},

    {24.995, 50.027, 84.021, 99.902, 199.805, 399.610, 8.000, 16.000, 32.000,
     64.000, 128.000, 256.000, 512.000, 1024.000, 2048.000, 4096.000},
};

bool PPG::init(struct k_msgq *queue) {
  if (!_active) {
    const struct gpio_dt_spec LDO_EN = {.port =
      ...
      if (to_write > 1) {
        ...
        for (int i = 0; i < to_write; i++) {
          memcpy(&msg_ppg.data.data[i * _size],
                 &sensor.data_buffer[written + i], _size);
        }
