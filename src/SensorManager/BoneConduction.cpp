...
const SampleRateSetting<10> BoneConduction::sample_rates = {
    {BMA5_ACC_ODR_HZ_12P5, BMA5_ACC_ODR_HZ_25, BMA5_ACC_ODR_HZ_50,
     BMA5_ACC_ODR_HZ_100, BMA5_ACC_ODR_HZ_200, BMA5_ACC_ODR_HZ_400,
     BMA5_ACC_ODR_HZ_800, BMA5_ACC_ODR_HZ_1K6, BMA5_ACC_ODR_HZ_3K2,
     BMA5_ACC_ODR_HZ_6K4}, // reg_vals

    {12.5, 25.0, 50.0, 100.0, 200.0, 400.0, 800.0, 1600.0, 3200.0,
     6400.0}, // sample_rates

    {12.5, 25.0, 50.0, 100.0, 200.0, 400.0, 800.0, 1600.0, 3200.0,
     6400.0} // true_sample_rates
};
...
void BoneConduction::update_sensor(struct k_work *work) {
  ...
  BoneConduction::sensor._sample_count +=
      (_time_stamp - BoneConduction::sensor._last_time_stamp) /
      BoneConduction::sensor.t_sample_us;
  BoneConduction::sensor._last_time_stamp = _time_stamp;

  if (BoneConduction::sensor._sample_count <
      BoneConduction::sensor._num_samples_buffered *
  ...
  if (num_samples > 0) {
    BoneConduction::sensor._sample_count =
        MAX(0, BoneConduction::sensor._num_samples_buffered - num_samples);
  }
  ...
  while (written < num_samples) {
    ...
    msg_bc.data.time = _time_stamp - (num_samples - written) *
                                         BoneConduction::sensor.t_sample_us;

    if (to_write > 1) {
      uint16_t t_diff = BoneConduction::sensor.t_sample_us;
      for (int i = 0; i < to_write; i++) {
        memcpy(&msg_bc.data.data[i * _size], &sensor.fifo_acc_data[written + i],
               _size);
      }
      memcpy(&msg_bc.data.data[msg_bc.data.size - sizeof(uint16_t)], &t_diff,
             sizeof(uint16_t));
    } else {
...
void BoneConduction::start(int sample_rate_idx) {
  ...
  bma580.init(sample_rates.reg_vals[sample_rate_idx],
              _num_samples_buffered * word_size);
  bma580.start();
